(function ($) {
    ...
    $.fn.extend({
        cronGen: function (options) {
            ...
            // Add an input
            var $i = $("<input>", { type: 'text',name:'jobCron', placeholder: 'cron表达式...',readOnly:'readOnly'}).addClass("form-control").val($(that).val());
            $i.appendTo($g);
            // Add the button
            ...
            $b.popover({
                ...
                if(clickNum==true) {
                    displayElement.attr("readOnly",clickNum);
                    clickNum=false;
                ...
                $("#CronGenMainDiv select,input[name!='jobCron']").on('input propertychange', function(e) {
                    generate();
                    $.fn.cronGen.tools.cronParse(displayElement.val());
                });
                $("#CronGenMainDiv select,input[name='jobCron']").on('input propertychange', function(e) {
